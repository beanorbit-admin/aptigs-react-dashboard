...
export default function SemesterList() {
  ...
  const { semesters, subjects, loading: contentLoading } = useAppSelector(s => s.courseContent)
  ...
  if (contentLoading && !course) return (
  ...
  return (
    <PageWrapper title="Course Content">
      ...
      {/* Search bar */}
      <div className="mb-5">
        <SearchInput value={search} onChange={setSearch} placeholder="Search semesters..." />
      </div>

      {/* Semester grid */}
      {contentLoading && courseSemesters.length === 0 ? (
        <CardSkeleton count={6} />
